...
// 密码加密密钥（与后端配置一致）
const PASSWORD_ENCRYPT_KEY = 'p-sandy-password'

/**
 * 加密密码（用于登录、新增/修改用户等场景）
 * 后端解密后会截取前13个字符作为时间戳，所以需要在密码前加上13位时间戳
 * @param password 原始密码
 * @returns 加密后的密码
 */
export const encryptPassword = (password: string): string => {
  ...
  return AES.encrypt(dataToEncrypt, PASSWORD_ENCRYPT_KEY)
}
...
export class AES {
  ...
  static encrypt(data: string, key: string): string {
    try {
      ...
      if (key.length !== 16 && key.length !== 32) {
        throw new Error(`AES 加密密钥长度必须为 16 或 32 位，当前长度: ${key.length}`)
      }
  ...
  static decrypt(encryptedData: string, key: string): string {
    try {
      ...
      if (key.length !== 16 && key.length !== 32) {
        throw new Error(`AES 解密密钥长度必须为 16 或 32 位，当前长度: ${key.length}`)
      }
      if (!encryptedData) {
