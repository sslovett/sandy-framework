...
let secretKey = ref(''), //后端返回的ase加密秘钥
  ...
  transitionWidth = ref(''),
  startLeft = ref(0),
  isInitialized = ref(false) // 防止重复初始化
...
const end = () => {
  ...
  if (status.value && isEnd.value == false) {
    ...
    reqCheck(data).then((res) => {
      // 后端返回格式: { code: "00000", data: { repCode: "0000", repData: {...} } }
      const checkData = res.data || res
      if (checkData.repCode == '0000') {
...
const getPictrue = async () => {
  ...
  if (captchaData.repCode == '0000') {
    backImgBase.value = captchaData.repData.originalImageBase64
    blockBackImgBase.value = captchaData.repData.jigsawImageBase64
    backToken.value = captchaData.repData.token
    secretKey.value = captchaData.repData.secretKey
  } else {
    tipWords.value = captchaData.repMsg
  }
}
</script>
